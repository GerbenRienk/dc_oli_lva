...
def cycle_through_syncs():
    ...
    while True:
        ...
        for one_response_data in responses_dict['responses']:
            ...
            # we should have the study subject id already in ls_responses, so get it from there:
            study_subject_id = conn.DLookup('study_subject_id', 'ls_responses', 'sid=%i and response_id=%i' % (sid, response_id))
            study_subject_oid = conn.DLookup('study_subject_oid', 'ls_responses', 'sid=%i and response_id=%i' % (sid, response_id))   
            # from here the code should be the same as in oli_lva
            if (study_subject_id is None or len(study_subject_id) != 13):
